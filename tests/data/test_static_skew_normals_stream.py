...
def test_stream_data(test_path):
    ds = StaticSkewNormalsDriftStream(250, 12, dist_support=[0, 11], random_state=1)

    test_file = os.path.join(test_path, 'static_skew_normals_stream.csv')
...
def test_stream_pdf(test_path):
    ...
    test_file = os.path.join(test_path, 'static_skew_normals_pdf.csv')
    raw_data = pd.read_csv(test_file, header=None)
